 ...
 *    
 *    GET : /user/100 ==> 100번 회원의 정보를 조회하는 로직 처리한다
...
public class AjaxRestController {
	...
	public MemoVO ajaxVO() {
		MemoVO vo = new MemoVO(55, "김철이잉", "반갑삼", null);
		return vo;
	}
	...
	//Path접근 방식으로 특정 번호를 파라미터로 받아 특정번호의 메모를json으로 반환해보자.
	...
	//json 데이터를 파라미터로 보내면 이것을 MemoVO로 받아 Map(=>json)으로 반환처리 => 스프링은 자동으로 Converting 한다
	...
	public Map<String, MemoVO> ajaxRestJson(@RequestBody MemoVO vo){
		...
		map.put("memo1", vo);
		map.put("memo2", new MemoVO(222,"이순식","나를 따르라",null));
		return map;
	}
